...
- Use the same word for the same concept, and call an FIU concept by the one name FIU uses for it, the name a colleague greps for: canonicalFamily, not ingredient family. Write service and register names exactly as the labels tool spells them, without asking; when a source uses another name, the body may mention it once. Spell out an abbreviation on first use: "recipe to basket (R2B)". No idiom, no metaphor, no marketing language.
...
- `service/`: a question that a colleague, a partner or a client actually asked, with its answer, stays; a question asked is the ticket in. The answer as it was given is the atom, hedges included, and the hedge is stated in the body ("FIU could not say whether ..."). Documentation copied without anyone having asked is out.
